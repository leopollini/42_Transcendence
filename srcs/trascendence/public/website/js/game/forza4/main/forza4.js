import { current_user, forza4_save, navigate, opponent, players } from "../../../main.js";
import { forza4CustomData } from "../data/forza4_game_global.js";
import { createGrid, redrawGrid } from "../board/forza4_grid.js";
import { updateTimer, resetTimer } from "../other/forza4_timer.js";
import { activatePowerup } from "../board/forza4_powerup.js";
import { remove_all } from "../../../error_main.js";

export let forza4GameData;
let backImageButton;
let matchPlayers = [];

export function Forza4() {
    return `
        <div id="forza4Game">
            <div id="f4players">
                <div id="p1" class="f4player-info">
                    <span class="f4-name" id="p1Name"></span>&nbsp;<span class="f4-token color token1" id="p1Color"></span>   
                </div>
                <button id="p1PowerupButton" class="f4-button-style" disabled>Power-up</button>
                <div id="p2" class="f4player-info">
                    <span class="f4-name" id="p2Name"></span>&nbsp;<span class="f4-token color token2" id="p2Color"></span>  
                </div>
                <button id="p2PowerupButton" class="f4-button-style" disabled>Power-up</button>
            </div>
            <div id="f4powerupinfo"></div>
            <div id="f4timer">00:00</div>
            <div id="f4grid-container">
                <div id="f4board"></div>
            </div>
            <div id="f4message"></div>
            <button id="f4BackToMenuButton" class="f4-button-style hidden-button">Back to Menu</button>
        </div>
        <div class="avatar-container">
            <img id="backImageButton" src="../website/images/home.png" alt="Back" class="back-button">
        </div>
    `;
}

export function startForza4Game() {
    //console.log("start forza 4 game");
    //console.log(players);
    backImageButton = document.getElementById('backImageButton');
    matchPlayers = players;
    console.log("forza4 save: ", forza4_save);
    if (forza4_save !== null && forza4_save !== undefined)
        forza4GameData = forza4_save;
    else
        forza4GameData = forza4CustomData;
    return new Forza4Game();
}

class Forza4Game {
    constructor() {
        this.p1Name = document.getElementById('p1Name');
        this.p2Name = document.getElementById('p2Name');
        this.p1Color = document.getElementById('p1Color');
        this.p2Color = document.getElementById('p2Color');
        this.f4BackToMenuButton = document.getElementById('f4BackToMenuButton');
        this.f4PowerupInfo = document.getElementById('f4powerupinfo');
        this.grid = document.getElementById('f4board');
        this.message = document.getElementById('f4message');
        this.moves = 0;
        this.elapsedTime = 0;

        this.p1Color.style.backgroundColor = forza4GameData.token1Color;
        this.p2Color.style.backgroundColor = forza4GameData.token2Color;
        this.rows = 6;
        this.cols = 7;
        this.currentPlayer = 'token1';
        this.p1PowerupUsed = false;
        this.p2PowerupUsed = false;
        this.powerUpActive = false;
        this.board = Array.from({ length: this.rows }, () => Array(this.cols).fill(null));
        this.gameEnded = false;

        this.p1 = current_user.display_name;
        this.p2 = opponent;
        //this.p2 = sessionStorage.getItem("opponent");

        this.p1Name.textContent = this.p1 + ":";
        this.p2Name.textContent = this.p2 + ":";

        // Bind handlers
        this.handleBackToMenu = this.handleBackToMenu.bind(this);
        this.handleBackButton = this.handleBackButton.bind(this);
        this.handleResize = this.handleResize.bind(this);
        this.handleP1Powerup = this.handleP1Powerup.bind(this);
        this.handleP2Powerup = this.handleP2Powerup.bind(this);
        this.handlePopState = this.handlePopState.bind(this);

        createGrid(this);
        this.printPlayerTurnMessage();

        this.startTime = Date.now();
        this.timerInterval = setInterval(() => updateTimer(this), 1000);

        this.addEventListeners();
        this.checkPowerups();
    }

    handleBackToMenu() {
        this.destroy();
        // old bg color-> #b97070
        document.getElementById("app").style.background = 
            "linear-gradient(35deg, #491f1f, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
        resetTimer(this);
        navigate("/modes", "Game Modes");
    }

    handleBackButton() {
        remove_all(1, 1);
        this.destroy();
        document.getElementById("app").style.background = 
            "linear-gradient(35deg, #491f1f, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
        resetTimer(this);
        navigate("/modes", "Return to Game Mode");
    }

    handleResize() {
        createGrid(this);
        redrawGrid(this, this.rows, this.cols);
    }

    handleP1Powerup() {
        activatePowerup(this, 'token1');
    }

    handleP2Powerup() {
        activatePowerup(this, 'token2');
    }

    handlePopState() {
        //console.log("popstate");
        matchPlayers = [];
        this.destroy();
        document.getElementById("app").style.background = 
            "linear-gradient(35deg, #491f1f, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
        resetTimer(this);
    }

    printPlayerTurnMessage() {
        if (this.currentPlayer === 'token1') {
            this.message.textContent = `Turno di ${this.p1}`;
        } else {
            this.message.textContent = `Turno di ${this.p2}`;
        }
    }

    checkPowerups() {
        if (!forza4GameData.powerUpMode) {
            document.getElementById('p1PowerupButton').style.display = "none";
            document.getElementById('p2PowerupButton').style.display = "none";
        } else {
            document.getElementById('p1PowerupButton').style.display = "block";
            document.getElementById('p2PowerupButton').style.display = "block";
            document.getElementById('p1PowerupButton').disabled = false;
            document.getElementById('p2PowerupButton').disabled = false;
            this.p1PowerupUsed = false;
            this.p2PowerupUsed = false;
        }
    }

    addEventListeners() {
        this.f4BackToMenuButton.addEventListener('click', this.handleBackToMenu);
        backImageButton?.addEventListener('click', this.handleBackButton);
        window.addEventListener("popstate", this.handlePopState);
        window.addEventListener('resize', this.handleResize);

        if (forza4GameData.powerUpMode) {
            document.getElementById('p1PowerupButton').addEventListener('click', this.handleP1Powerup);
            document.getElementById('p2PowerupButton').addEventListener('click', this.handleP2Powerup);
        }
    }

    destroy() {
        // Clear timer
        if (this.timerInterval) {
            clearInterval(this.timerInterval);
            this.timerInterval = null;
        }

        // Remove event listeners
        if (this.f4BackToMenuButton) {
            this.f4BackToMenuButton.removeEventListener('click', this.handleBackToMenu);
        }
        
        if (backImageButton) {
            backImageButton.removeEventListener('click', this.handleBackButton);
        }

        window.removeEventListener("popstate", this.handlePopState);
        window.removeEventListener('resize', this.handleResize);

        // Clean up powerup buttons if in powerup mode
        if (forza4GameData.powerUpMode) {
            const p1PowerupButton = document.getElementById('p1PowerupButton');
            const p2PowerupButton = document.getElementById('p2PowerupButton');
            
            if (p1PowerupButton) {
                p1PowerupButton.removeEventListener('click', this.handleP1Powerup);
            }
            if (p2PowerupButton) {
                p2PowerupButton.removeEventListener('click', this.handleP2Powerup);
            }
        }

        // Clear game board
        if (this.grid) {
            this.grid.innerHTML = '';
        }

        // Reset game state
        this.gameEnded = true;
        this.board = Array.from({ length: this.rows }, () => Array(this.cols).fill(null));

        // Clear messages
        if (this.message) {
            this.message.textContent = '';
        }

        if (this.f4PowerupInfo) {
            this.f4PowerupInfo.textContent = '';
        }
    }
}










// import { current_user, navigate} from "../../../main.js";
// import { token1Color, token2Color, powerUpMode} from "../data/forza4_game_global.js";
// import { createGrid, redrawGrid } from "../board/forza4_grid.js";
// import { updateTimer, resetTimer } from "../other/forza4_timer.js";
// import { activatePowerup } from "../board/forza4_powerup.js";

// let backImageButton;
// let matchPlayers = [];
// export function Forza4() {

//     return `
//             <div id="forza4Game">
//             <div id="f4players">
//                 <div id="p1" class="f4player-info">
//                     <span class="f4-name" id="p1Name"></span>&nbsp;<span class="f4-token color token1" id="p1Color"></span>   
//                 </div>
//                 <button id="p1PowerupButton" class="f4-button-style" disabled>Power-up</button>
//                 <div id="p2" class="f4player-info">
//                     <span class="f4-name" id="p2Name"></span>&nbsp;<span class="f4-token color token2" id="p2Color"></span>  
//                 </div>
//                 <button id="p2PowerupButton" class="f4-button-style" disabled>Power-up</button>
//             </div>
//             <div id="f4powerupinfo"></div>
//             <div id="f4timer">00:00</div>
//             <div id="f4grid-container">
//                 <div id="f4board"></div> <!-- Griglia di gioco -->
//             </div>
//             <div id="f4message"></div>
//             <button id="f4BackToMenuButton" class="button-style">Back to Menu</button>
//         </div>
//         <div class="avatar-container">
//             <img id="backImageButton" src="../website/images/home.png" alt="Back" class="back-button">
//         </div>
//         `;
// }

// export function startForza4Game(players) {
//     console.log("start forza 4 game");
//     backImageButton = document.getElementById('backImageButton');
//     matchPlayers = players;
//     if (!sessionStorage.getItem("f4opponent"))
//         sessionStorage.setItem("f4opponent", players[1]);
//     let f4Game = new Forza4Game();
//     //console.log("playerzz:" +players);
   
//     //console.log("match player 1:", matchPlayers[0]);
//     //console.log("match player 2:", matchPlayers[1]);
// }

// class Forza4Game {

//     constructor() {
//         this.p1Name = document.getElementById('p1Name');
//         this.p2Name = document.getElementById('p2Name');
//         this.p1Color = document.getElementById('p1Color');
//         this.p2Color = document.getElementById('p2Color');
//         this.f4BackToMenuButton = document.getElementById('f4BackToMenuButton');
//         this.f4PowerupInfo = document.getElementById('f4powerupinfo');
//         this.grid = document.getElementById('f4board');
//         this.message = document.getElementById('f4message');
//         this.moves = 0;
//         this.elapsedTime = 0;

//         this.p1Color.style.backgroundColor = token1Color;
//         this.p2Color.style.backgroundColor = token2Color;
//         this.rows = 6;
//         this.cols = 7;
//         this.currentPlayer = 'token1';
//         this.p1PowerupUsed = false;
//         this.p2PowerupUsed = false;
//         this.powerUpActive = false;
      
//         this.board = Array.from({ length: this.rows }, () => Array(this.cols).fill(null));
//         this.gameEnded = false;

//         //this.f4Players = JSON.parse(sessionStorage.getItem('forza4players'));
//         this.p1 = current_user.display_name;
//         this.p2 = sessionStorage.getItem("f4opponent");

//         this.p1Name.textContent = this.p1 + ":";
//         this.p2Name.textContent = this.p2 + ":";

//         createGrid(this);
//         this.printPlayerTurnMessage();

//         this.startTime = Date.now();
//         this.timerInterval = setInterval(() => updateTimer(this), 1000);

        
//         this.addEventListeners();
//         this.checkPowerups();

//     }

//     printPlayerTurnMessage() {
//         if (this.currentPlayer === 'token1') {
//             this.message.textContent = `Turno di ${this.p1}`;
//         }
//         else {
//             this.message.textContent = `Turno di ${this.p2}`;
//         }
//     }

//     checkPowerups() {
//         if (!powerUpMode) {
//             document.getElementById('p1PowerupButton').style.display = "none";
//             document.getElementById('p2PowerupButton').style.display = "none";
            
//         }
//         else
//         {
//             document.getElementById('p1PowerupButton').style.display = "block";
//             document.getElementById('p2PowerupButton').style.display = "block";
//             document.getElementById('p1PowerupButton').disabled = false;
//             document.getElementById('p2PowerupButton').disabled = false;
//             this.p1PowerupUsed = false;
//             this.p2PowerupUsed = false;
    
//             // Power-up buttons listeners
//             document.getElementById('p1PowerupButton').addEventListener('click', () => activatePowerup(this, 'token1'));
//             document.getElementById('p2PowerupButton').addEventListener('click', () => activatePowerup(this, 'token2'));
//         } 
           
//     }


//     addEventListeners() {
//         f4BackToMenuButton.addEventListener('click', () => {
//             sessionStorage.setItem("game ended", false);
//             document.getElementById("app").style.background = 
//             "linear-gradient(35deg, #b97070, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
//             resetTimer(this);
//             navigate("/modes", "Game Modes");
//         });
    
//         backImageButton?.addEventListener('click', () => {
//             document.getElementById("app").style.background = 
//     "linear-gradient(35deg, #491f1f, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
//             resetTimer(this);
//             navigate("/modes", "Return to Game Mode");
//         });
//         window.addEventListener("popstate", (event) => {
//             document.getElementById("app").style.background = 
//             "linear-gradient(35deg, #491f1f, #134946), radial-gradient(circle, rgba(255, 243, 255, 0.2) 30%, transparent 60%)";
//            resetTimer(this);
//         });
        
//         // Recalculate grid size on window resize
//         window.addEventListener('resize', () => {
//             createGrid(this);
//             redrawGrid(this, this.rows, this.cols);
//         });
//     }
// }

