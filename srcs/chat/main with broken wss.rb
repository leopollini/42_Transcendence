  ...
  if Faye::WebSocket.websocket?(req)
    ...
    ws.on :message do |event|
      begin
        ...
        # return ws.close if data['username'].to_s == 'default'
        ...
        when "join"
          ...
          if ChatStore.exists? @username
            ws.puts({'status' => 'another user with the same username is already connected', 'succes' => 'false', 'type' => 'kick'}.to_json)
            socket_close(ws)
            return
          end
          @token = data["token"].to_s
          ChatStore.joined @username, ws
