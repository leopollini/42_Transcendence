...
cert_path = "/var/www/ssl_certs/server.crt"
key_path = "/var/www/ssl_certs/server.key"
...
EM::WebSocket.start({
  ...
  sock.onclose do
    ChatStore.close_client(username) if username && !kicked
    ...
    when "send_message"
      ...
      if data["chat"].to_s == "general"
        ...
        unless ChatStore.clients[username].friends.include? target
          next ChatStore.clients[username].send_sys "The message could not be delivered" 
        end
